...
img = Image.open('covid-icn.png')
st.set_page_config(page_title='E-Commerce Analysis', page_icon=img, layout='wide')

# Background image
img = get_img_as_base64("covid-19.jpg")
...
def global_section():
                    ...
                    "series": [
                        {
                            ...
                            "data": [
                                {"value": int(sizes[0]), "name": labels[0]},
                                {"value": int(sizes[1]), "name": labels[1]},
                                {"value": int(sizes[2]), "name": labels[2]},
                                {"value": int(sizes[3]), "name": labels[3]},
                            ]
                        }
                    ]
                }
    ...
    # Section 2: COVID-19 Statistics Over Time
        ...
        # Create a pie chart for the latest date    
            ...
            labels = list(latest_data.index)
            sizes = list(latest_data.values)
